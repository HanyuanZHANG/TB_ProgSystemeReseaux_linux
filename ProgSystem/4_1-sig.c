...
void hdl_sys1(int n) {
  ...
  ancien = act2; //using sigaction
}
void hdl_sys2(int n) {
  printf("message numero 2\n");
  ancien = act;  // using sigaction
}

void hdl_sysQuit(int n) {
  //sig_avant = signal(SIGINT, sig_avant); //using signal
  sigaction(SIGINT, &ancien, SIG_DFL); // using sigaction
}
